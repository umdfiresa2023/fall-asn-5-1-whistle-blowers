...
```{r}
ggplot(df, aes(x = ESG_Score, y = GHG.QUANTITY..METRIC.TONS.CO2e., col=company)) +
  geom_point() +
  labs(x = "esg", y = "GHG", title = "esg vs GHG") +
...
modelscore <-lm(ESG_Score ~ GHG.QUANTITY..METRIC.TONS.CO2e., data = df)
```
...
$Y_{it}$ represent the outcome variable GHG for each company (geographical unit) $i$, and year (time unit) $t$.

Based on the regression from Question 6, $\beta_0$ is equal to .38

Based on the regression from Question 6, $\beta_1$ is equal to 1.76\*10\^-8
...
```{r}
modelscore2 <-lm(ESG_Score ~ GHG.QUANTITY..METRIC.TONS.CO2e. + year, data = df)
get_regression_table(modelscore2)
```
...
```{r}
get_regression_points(modelscore2) %>%
...
summary(model<-felm(ESG_Score ~ GHG.QUANTITY..METRIC.TONS.CO2e. + year | company , data = df))
```
...
df2 <- df2 %>%
  mutate(sq_residuals = ESG_Score^2) %>%
  summarize(sum_sq_residuals = sum(sq_residuals))
...
ESG_Score=1.406×10\^−8 × GHG.QUANTITY..METRIC.TONS.CO2e. + 0.01557 × year
...
df <- df %>%
      mutate(ESG_Size_Interaction = interaction(ESG_Year_Interaction, Company_Size_Category),
             Policy_Year_Trend = interaction(Environmental_Policy, Year_Trend))

```

Answer:\
We can combine the **`ESG_Year_Interaction`** variable with the **`Company_Size_Category`** to analyze how the impact of ESG scores over time varies with company size. This will reveal if larger companies are more responsive to ESG standards over time compared to smaller ones.

Additionally, we can merge the **`Environmental_Policy`** dummy variable with the **`Year_Trend`** categorical variable. This will help us understand if the adoption of environmental policies has a more pronounced effect on GHG emissions in certain time periods.
...
An alternative regression that you can run is:

```{r}
summary(model<-felm(ESG_Score ~ GHG.QUANTITY..METRIC.TONS.CO2e. | company + year, data = df))
```
